...
import { InventoryItem, forecastInventoryMovingAverage, INVENTORY_MODELS } from '@/lib/inventory/forecaster';
  ...
  return (
    <div className="min-h-screen p-8 bg-gradient-to-br from-amber-50 to-yellow-100 dark:from-gray-900 dark:to-gray-800">
      <div className="max-w-4xl mx-auto">
        ...
        <p className="text-gray-600 dark:text-gray-400 mb-8">Forecast inventory needs to reduce waste and prevent stockouts</p>
          ...
          <div className="grid grid-cols-3 gap-4">
            {Object.entries(INVENTORY_MODELS).map(([key, model]) => (
              <div key={key} className="p-4 border rounded">
                ...
                <p className="text-xs text-gray-600 dark:text-gray-400 mt-2">{model.waste}% waste</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
          <h2 className="text-2xl font-bold mb-4 text-gray-900 dark:text-gray-100">Inventory Forecast</h2>
          <div className="space-y-4">
            {forecasts.map((forecast, idx) => (
              <div key={idx} className="p-4 border rounded">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <p className="font-semibold text-gray-900 dark:text-gray-100">{forecast.item.name}</p>
                    <p className="text-sm text-gray-600 dark:text-gray-400">Current Stock: {forecast.item.currentStock}</p>
                  </div>
                  <span className={`px-3 py-1 rounded text-sm ${
                    forecast.daysUntilStockout < 3 ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300'
                    : forecast.daysUntilStockout < 7 ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300'
