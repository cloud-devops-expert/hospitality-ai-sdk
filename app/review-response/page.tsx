...
export default function ReviewResponsePage() {
  ...
  const [reviewText, setReviewText] = useState('The room was absolutely amazing! The staff were incredibly helpful and the location was perfect. Highly recommend!');
  ...
  const examples = [
    { rating: 5, text: 'Amazing stay! Clean room, friendly staff, perfect location. Will definitely return!' },
    { rating: 2, text: 'Very disappointed. Room was dirty, wifi didn\'t work, and staff were unhelpful.' },
    { rating: 3, text: 'Decent hotel. Good location but room was smaller than expected. Staff were nice though.' },
  ];
  ...
  const handleGenerate = () => {
    ...
    // For now, only template is implemented
  ...
  const getSentimentColor = (sentiment: string) => {
    switch (sentiment) {
      case 'positive': return 'text-green-600 bg-green-100 dark:text-green-400 dark:bg-green-900/30';
      case 'negative': return 'text-red-600 bg-red-100 dark:text-red-400 dark:bg-red-900/30';
      case 'mixed': return 'text-yellow-600 bg-yellow-100 dark:text-yellow-400 dark:bg-yellow-900/30';
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {(['template', 'rag', 'llm'] as const).map((algo) => {
              ...
              const isAvailable = algo === 'template';
              return (
                <button
                  key={algo}
                ...
                >
                  ...
                  <div className="text-xs space-y-1 text-gray-600 dark:text-gray-400">
                    ...
                    <div><strong>Quality:</strong> {(info.quality * 100).toFixed(0)}%</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
            <h2 className="text-2xl font-bold mb-4 text-gray-900 dark:text-gray-100">Review Details</h2>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-2 text-gray-700 dark:text-gray-300">Rating: {rating} stars</label>
                <input type="range" value={rating} onChange={(e) => setRating(Number(e.target.value) as 1 | 2 | 3 | 4 | 5)}
              ...
              <div>
                ...
                <select value={platform} onChange={(e) => setPlatform(e.target.value as Review['platform'])}
                  className="w-full p-2 border rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100">
              ...
              <div>
                <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">Review Text</label>
              ...
              <div className="border-t pt-4">
                ...
                {examples.map((ex, idx) => (
                  <button key={idx} onClick={() => { setRating(ex.rating as 1 | 2 | 3 | 4 | 5); setReviewText(ex.text); }}
          ...
          <div className="space-y-6">
            {result && (
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
                <h2 className="text-2xl font-bold mb-4 text-gray-900 dark:text-gray-100">Generated Response</h2>

                <div className="bg-gray-50 dark:bg-gray-900 p-4 rounded mb-4">
                  <p className="text-gray-800 dark:text-gray-200 leading-relaxed">{result.draftResponse}</p>
                </div>

                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div className="bg-gray-50 dark:bg-gray-900 p-3 rounded">
                    <p className="text-xs text-gray-600 dark:text-gray-400">Sentiment</p>
                    <p className={`text-sm font-bold px-2 py-1 rounded inline-block ${getSentimentColor(result.sentiment)}`}>
                      {result.sentiment.toUpperCase()}
                    </p>
                  </div>
                  <div className="bg-gray-50 dark:bg-gray-900 p-3 rounded">
                    <p className="text-xs text-gray-600 dark:text-gray-400">Tone</p>
                    <p className="text-sm font-bold capitalize text-gray-900 dark:text-gray-100">{result.tone}</p>
                  </div>
                </div>

                <div className="mb-4">
                  <p className="text-sm font-medium mb-2 text-gray-700 dark:text-gray-300">Key Topics Detected</p>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-gray-50 dark:bg-gray-900 p-3 rounded">
                    ...
                    <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">{(result.quality * 100).toFixed(0)}%</p>
                  </div>
                  <div className="bg-gray-50 dark:bg-gray-900 p-3 rounded">
                    <p className="text-xs text-gray-600 dark:text-gray-400">Processing Time</p>
                    <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">{result.processingTime}ms</p>
                  </div>
                </div>
              </div>
            )}
