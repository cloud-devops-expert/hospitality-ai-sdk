...
describe('Staff Scheduling Optimization', () => {
  ...
  describe('Role Distribution', () => {
    it('should distribute different roles across shifts', () => {
      ...
      const allRoles = result.shifts.flatMap((shift) => shift.staff.map((member) => member.role));

      // Should have at least some roles represented
      expect(allRoles.length).toBeGreaterThan(0);
      expect(
        allRoles.some((role) => ['front-desk', 'housekeeping', 'maintenance'].includes(role))
      ).toBe(true);
    });
  });
      ...
      const limitedAvailability: StaffMember[] = [
        {
          ...
          role: 'front-desk',
          hoursPerWeek: 40,
          availability: [true, false, false, false, false, false, false],
        },
        {
          id: 's2',
          name: 'Test2',
          role: 'housekeeping',
          hoursPerWeek: 40,
          availability: [false, true, false, false, false, false, false],
        },
      ];
